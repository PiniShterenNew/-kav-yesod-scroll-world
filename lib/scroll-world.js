...
function createFrameSequenceRenderer(opts) {
  ...
  const state = { active: false, initialized: false, failed: false, manifest: null, gsap: null, ScrollTrigger: null,
    ...
    targetF: 0, displayF: 0, animRaf: 0, animLast: 0, srcW: 0, srcH: 0, pendingPriority: new Set() };
  ...
  function init() {
    ...
    loadStill(0).then((img) => {
      if (generation !== state.generation || !img) return;
      drawImageCover(img);
      // Remember the poster: the flight's frame 0 is a different render, so
      // once it arrives we dissolve into it instead of popping (see render()).
      state.introStill = img;
    });
    Promise.all([
      ...
      state.srcW = manifest.width || 0;
      state.srcH = manifest.height || 0;
      ...
      // Show the first flight's frame 0 as the resting hero (the station
      // frames ARE the flight endpoints; the poster stills are separate
      // renders that don't match them — showing frame 0 up front means no
      // content jump when scrolling starts).
      if (!state.current) {
        state.current = { transition: state.manifest.transitions[0] };
        state.targetF = 0;
        state.displayF = 0;
        queueFrame(state.manifest.transitions[0], 0, true);
        // queueFrame no-ops on a cache hit, so schedule the draw explicitly —
        // render() holds the poster until frame 0 is actually available.
        scheduleRender();
      }
      layout();
  ...
  function createTriggers() {
    ...
    state.manifest.transitions.forEach((transition, index) => {
      ...
      if (!segment) return;
      // Station-then-flight rhythm: the first ~45% of each segment rests on a
      // crisp still of the stage (frame 0, with the copy readable and the
      // route/status showing WHERE you are), and only then the flight to the
      // next stage plays through the remaining scroll. Scroll = image, then
      // video, section by section.
      const hold = index === 0 ? 0.52 : 0.42;
  ...
  // Scroll progress -> frame position through the transition's motion profile
  // (when present): equal scroll = equal VISUAL change. The source renders'
  // camera speed isn't uniform — lurches and near-freezes are baked into the
  // videos — so a linear time mapping makes scrubbing feel jerky no matter how
  // smooth the engine is. Inverting the cumulative-motion curve stretches the
  // lurches over more scroll and skips the freezes.
  function frameFromProgress(transition, p) {
    const motion = transition.motion;
    const last = (transition.frameCount || transition.frames.length) - 1;
    if (!Array.isArray(motion) || motion.length !== last + 1) return p * last;
    let lo = 0, hi = last;
    while (lo < hi) { const mid = (lo + hi) >> 1; if (motion[mid] < p) lo = mid + 1; else hi = mid; }
    if (lo === 0) return 0;
    const a = motion[lo - 1], b = motion[lo];
    return (lo - 1) + (b > a ? (p - a) / (b - a) : 1);
  }

  function setTransition(transition, progress) {
    if (!state.active) return;
    const target = frameFromProgress(transition, clamp(progress, 0, 1));
  ...
  function animTick(now) {
    ...
    if (Math.abs(diff) < 0.02) {
      // Park ON a whole frame: a fractional resting position would leave a
      // permanent 2-frame blend on screen — a visibly soft "ghosted" image
      // whenever the user stops scrolling. The ≤half-frame settle is
      // imperceptible at 24fps source spacing.
      state.displayF = Math.round(state.targetF);
      render();
  ...
  function queueFrame(transition, frameIndex, priority) {
    ...
    if (state.cache.has(url) || state.failedUrls.has(url)) return;
    // Already downloading at low priority? Upgrade it: the render that's
    // waiting on it must be scheduled when it lands.
    if (state.pending.has(url)) {
      if (priority) state.pendingPriority.add(url);
      return;
    }
    const img = new Image();
    img.decoding = 'async';
    img.onload = () => {
      ...
      const finalize = () => {
        ...
        pruneCache(url);
        if (priority || state.pendingPriority.delete(url)) scheduleRender();
      };
      if (img.decode) img.decode().then(finalize, finalize); else finalize();
    };
    img.onerror = () => {
      state.pending.delete(url);
      state.pendingPriority.delete(url);
  ...
  function render() {
    ...
    // One-off intro: dissolve from the poster still into the flight's frame 0
    // (different renders — a hard swap would visibly pop at load). Any real
    // scrolling cancels it and frames take over directly.
    if (state.introStill) {
      if (state.targetF > 0.5 || transition.index !== 0) {
        state.introStill = null;
      } else {
        const first = getCached(transition, 0);
        if (!first) return;   // keep the still until frame 0 arrives
        const still = state.introStill;
        state.introStill = null;
        const startT = performance.now();
        const fade = (now) => {
          if (!state.active || state.targetF > 0.5) { scheduleRender(); return; }
          const t = Math.min(1, (now - startT) / 400);
          drawImageCover(still);
          blitCover(first.img, t);
  ...
  function canvasSize() {
    // Match the buffer to the SOURCE resolution: enough pixels that frames map
    // ~1:1 (maximum sharpness), and no more — extra dpr past that adds zero
    // detail and only inflates the blend pass's fill cost.
    const cssW0 = state.viewW || window.innerWidth;
    const cssH0 = state.viewH || window.innerHeight;
    const srcCap = state.srcW ? Math.max(1, Math.min(state.srcW / cssW0, state.srcH / cssH0)) : 1.5;
    const dpr = Math.min(window.devicePixelRatio || 1, 2, srcCap);
    const pixelW = Math.max(1, Math.round(cssW0 * dpr));
    const pixelH = Math.max(1, Math.round(cssH0 * dpr));
    if (canvas.width !== pixelW || canvas.height !== pixelH) {
      canvas.width = pixelW;
      canvas.height = pixelH;
    }
    return { pixelW, pixelH, cssH: cssH0, dpr };
  }
  ...
  // The canvas is lvh-tall and only clipped by the stage when the footer
  // reveals — so the image must be composed into the VISIBLE strip (viewport
  // minus footer), not the full buffer. Otherwise the clip shows the sky-top
  // of a full-height draw and the house slides out of view behind the footer.
  function visibleStrip(pixelH, dpr) {
    return Math.max(1, pixelH - Math.round((state.footerReveal || 0) * dpr));
  }

  // Base pass: opaque cover draw (fills every visible pixel — no clear needed).
  function drawImageCover(img) {
    if (!img || !ctx) return;
    const { pixelW, pixelH, cssH, dpr } = canvasSize();
    const stripH = visibleStrip(pixelH, dpr);
    const r = coverRect(img, pixelW, stripH, cssH);
    ctx.globalAlpha = 1;
    ctx.drawImage(img, r.sx, r.sy, r.sw, r.sh, 0, 0, pixelW, stripH);
    if (state.active) {
      container.dataset.swSequence = label;
      container.classList.add('sw-has-sequence');
    }
