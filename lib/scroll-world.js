...
function createFrameSequenceRenderer(opts) {
  ...
  const state = { active: false, initialized: false, failed: false, manifest: null, gsap: null, ScrollTrigger: null,
    ...
    fallbackScene: 0, renderRaf: 0, resizeRaf: 0, generation: 0, footerReveal: 0, viewW: 0, viewH: 0 };
  // Cache holds compressed Image objects (~70KB each); two full transitions'
  // worth so a scroll-back never refetches the sequence it just played.
  const maxCache = 132;
  ...
  function init() {
    ...
    Promise.all([
      ...
      prefetchWindow(state.manifest.transitions[0], 0, 14);
      layout();
  ...
  function createTriggers() {
    ...
    state.manifest.transitions.forEach((transition, index) => {
      ...
      const trigger = state.ScrollTrigger.create({
        ...
        // scrub with a catch-up time (not `true`): raw wheel input moves in
        // ~100px notches, which at 61 frames/segment snaps ~5 frames per notch.
        // The 0.7s lerp lets GSAP interpolate through the in-between frames at
        // display rate, which is what makes the flight read as continuous.
        scrub: 0.7,
  ...
  function setTransition(transition, progress) {
    ...
    if (progress > 0.6 && state.manifest.transitions[transition.index + 1]) {
      prefetchWindow(state.manifest.transitions[transition.index + 1], 0, 12);
    }
    scheduleRender();
  }

  function prefetchWindow(transition, frameIndex, radius = 12) {
