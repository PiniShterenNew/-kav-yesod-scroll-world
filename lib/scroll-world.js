...
function createFrameSequenceRenderer(opts) {
  const { label, manifestUrl, canvas, container, scenes, segments, reduce, isActiveViewport, stillUrl, clamp, lingerEase } = opts;
  const transitionIds = [
  ...
  function createTriggers() {
    ...
    state.manifest.transitions.forEach((transition, index) => {
      ...
      // Per-section dwell (config `linger`): same monotone remap the video path
      // uses — the flight settles mid-scene where the copy peaks. f(0)=0, f(1)=1,
      // so seam frames are untouched.
      const remap = (p) => segment.linger && lingerEase ? lingerEase(p, segment.linger) : p;
      const trigger = state.ScrollTrigger.create({
        ...
        // The lerp lets GSAP glide through the in-between positions at display
        // rate; render() then cross-blends adjacent frames for the sub-frame
        // remainder, which is what makes a 12fps sequence read as continuous.
        scrub: 1,
        invalidateOnRefresh: true,
        onEnter: () => setTransition(transition, 0),
        onEnterBack: () => setTransition(transition, 1),
        onUpdate: (self) => setTransition(transition, remap(self.progress))
      });
  ...
  function setTransition(transition, progress) {
    ...
    // Fractional frame position — render() cross-blends floor/ceil frames by the
    // remainder instead of rounding, so motion between 12fps source frames is
    // continuous rather than stepped.
    const frameFloat = clamp(progress, 0, 1) * (frameCount - 1);
    const frameIndex = Math.round(frameFloat);
    state.current = { transition, frameFloat, frameIndex };
    state.fallbackScene = Math.min(transition.index + (progress > 0.78 ? 1 : 0), scenes.length - 1);
    queueFrame(transition, Math.floor(frameFloat), true);
    queueFrame(transition, Math.ceil(frameFloat), true);
  ...
  function render() {
    ...
    const { transition, frameFloat, frameIndex } = state.current;
    // Temporal blend: draw the frame below the fractional position, then the
    // frame above it at the remainder's alpha. At 12fps adjacent frames differ
    // a lot; this dissolve is what turns the per-frame step into fluid motion.
