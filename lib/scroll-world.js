/* ============================================================================
   scroll-world Ã¢â‚¬â€ portable scroll-scrubbed camera-flight engine
   ----------------------------------------------------------------------------
   ...
   USAGE
     mountScrollWorld(document.getElementById('world'), {
       ...
       opening: { Ã¢â‚¬Â¦ },    // optional hero scene (same shape as a section) shown before
                          // the numbered sections Ã¢â‚¬â€ no route dot, nav entry or NN/NN
       sections: [
         { id, label, still, stillMobile, clip, clipMobile, accent,
           scroll: 1.6,   // optional per-section override of diveScroll Ã¢â‚¬â€ more scroll
                          // distance = a slower, longer dwell in this scene
           linger: 0.5,   // optional 0..1 Ã¢â‚¬â€ remaps time so the camera settles mid-scene
                          // (exactly where the copy peaks) and moves quicker at the
                          // edges. 0 = linear (default). Keep Ã¢â€°Â¤ 0.6; 1 = full pause.
           eyebrow, title, body, tags:[Ã¢â‚¬Â¦],
           cta:{ primary:{label,href}, secondary:{label,href} } }, // last section only
         Ã¢â‚¬Â¦
       ],
       connectors: [clipUrl, Ã¢â‚¬Â¦],          // length = sections.length - 1 (nulls allowed)
       connectorsMobile: [clipUrl, Ã¢â‚¬Â¦],    // optional lighter connectors for phones (same length)
   ...
   the rest of the phone handling below is always on)
     The engine is phone-aware out of the box: on a coarse-pointer / Ã¢â€°Â¤860px viewport it
       - loads `clipMobile` / `connectorsMobile` when provided (encode these smaller +
         tighter-GOP Ã¢â‚¬â€ seek cost on a phone decoder is dominated by frames-from-keyframe,
       ...
       - keeps the still as a live poster until the clip actually paints its first frame,
         and primes each video (muted playÃ¢â€ â€™pause) on first touch Ã¢â‚¬â€ this is what stops iOS
     ...
     Nothing here is required Ã¢â‚¬â€ a config with only `clip`/`connectors` still works on
...
function mountScrollWorld(container, config) {
  ...
  // the Ã¢â€°Â¤860px query is read live via isMobile() so a desktop resize/DevTools toggle
  ...
  // NN/NN number â€” navigation stays 01/N over the sections alone.
  ...
  // ---- build the interleaved segment chain: dive0, conn0, dive1, Ã¢â‚¬Â¦ diveN-1 ----
  const SEGMENTS = [];
  SCENES.forEach((s, i) => {
  ...
  // route dots / nav â€” numbered sections only
  SECTIONS.forEach((s, i) => {
    const dot = el('button', 'sw-route__dot'); dot.type = 'button'; dot.setAttribute('aria-label', '???? ????? ' + pad(i + 1) + ': ' + (s.label || '')); dot.style.setProperty('--sw-accent', s.accent || '');
  ...
  // Per-section dwell: monotone remap of scrollÃ¢â€ â€™time so the camera settles mid-scene
  ...
  function setFooterReveal(px) {
    ...
    sequenceRenderers.forEach(renderer => renderer.handleMediaResize());
  }
  ...
  function layout() {
    ...
    totalW = off;
    track.style.height = (totalW * vh + (1 + OUTRO) * vh) + 'px';   // +1vh so the last flight completes, +OUTRO so the footer only enters after the final hold
    footerReveal = -1;
    setFooterReveal(0);
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: seg.start + (seg.end - seg.start) * 0.08, behavior: reduce ? 'auto' : 'smooth' });
  }

  function loadClip(s) {
    // Under prefers-reduced-motion we never load the clips at all Ã¢â‚¬â€ the stills stay up
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        // painted Ã¢â‚¬â€ on iOS a seeked-but-never-played muted video stays blank, so
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      let outside = 0;
      // The last scene never fades past its end â€” it stays up through the outro
    ...
    const footerVisible = footerHeight ? clamp((y + vh - track.offsetHeight) / footerHeight, 0, 1) : 0;
    setFooterReveal(footerHeight * footerVisible);
  ...
  // first touch we prime every loaded clip (muted playÃ¢â€ â€™pause) so the first seek is
...
function createFrameSequenceRenderer(opts) {
  ...
  const state = { active: false, initialized: false, failed: false, manifest: null, gsap: null, ScrollTrigger: null,
    ...
    fallbackScene: 0, renderRaf: 0, resizeRaf: 0, generation: 0 };
  const maxCache = 54;
  ...
  function createTriggers() {
    ...
    state.manifest.transitions.forEach((transition, index) => {
      ...
      const trigger = state.ScrollTrigger.create({
        ...
        onEnter: () => setTransition(transition, 0),
        onEnterBack: () => setTransition(transition, 1),
  ...
  function setTransition(transition, progress) {
    ...
    state.current = { transition, frameIndex };
    state.fallbackScene = Math.min(transition.index + (progress > 0.78 ? 1 : 0), scenes.length - 1);
    queueFrame(transition, frameIndex, true);
    prefetchWindow(transition, frameIndex);
    if (progress > 0.72 && state.manifest.transitions[transition.index + 1]) {
      prefetchWindow(state.manifest.transitions[transition.index + 1], 0, 8);
    }
    scheduleRender();
  }

  function prefetchWindow(transition, frameIndex, radius = 10) {
  ...
  function render() {
    ...
    const { transition, frameIndex } = state.current;
    const entry = nearestCached(transition, frameIndex);
    if (entry) {
  ...
  function drawImageCover(img) {
    ...
    const sx = (iw - sw) / 2;
    const footerReveal = parseFloat(getComputedStyle(container).getPropertyValue('--sw-footer-reveal')) || 0;
    const alignY = footerReveal > 0 ? 1 : 0.42;
...
function injectCSS() {
  ...
  // :root / .sw-root { --sw-bg / --sw-ink / --sw-accent Ã¢â‚¬Â¦ }) always win over
